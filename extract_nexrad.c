...
//
// where:
//   <type>          - Regional (for type 63) or CONUS (for type 64)
//   <hour>:<minute> - the time from the PDU header; all blocks from one composite radar image will have the same time
//   <scale>         - the scale value of this block:
//                     - 0 (high res)
//                     - 1 (med res)
//                     - 2 (low res)
//   <north>         - the north edge of the block, in _integer arcminutes_. Divide by 60 to get degrees.
//   <west>          - the west edge of the block, in _positive integer arcminutes_. Divide by 60 to get degrees;
//                     subtract 360 if you want the conventional -180..+180 range.
//   <height>        - the height of the block, in integer arcminutes of latitude
//   <width>         - the width of the block, in integer arcminutes of longitude
//
// Each block contains 128 evenly spaced bins, in a grid of 32 (longitude) x 4 (latitude), working west-to-east then
// north-to-south. i.e. each bin represents a pixel that covers <width>/32 arcminutes of longitude by <height>/4
// arcminutes of latitude.
//
//   <data> - a string of 128 digits (no spaces); each character represents the intensity of one bin, in the order above
//
//
// Given:
//   bn - block number
...
//   sf - scale factor
//
// compute the northwest corner of the referenced block and place it in (*latN, *lonW) and place the total height and
// width of the block in (*latSize, *lonSize)
