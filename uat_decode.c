...
static void uat_display_sv(const struct uat_adsb_mdb *mdb, FILE *to) {
	...
	if (mdb->position_valid) {
		fprintf(to, " Latitude:          %+.4f\n Longitude:         %+.4f\n", mdb->lat, mdb->lon);
	}

	switch (mdb->altitude_type) {
		case ALT_BARO:
			fprintf(to, " Altitude:          %d ft (barometric)\n", mdb->altitude);
			break;

		case ALT_GEO:
			fprintf(to, " Altitude:          %d ft (geometric)\n", mdb->altitude);
			break;
	...
	if (mdb->ns_vel_valid) {
		fprintf(to, " N/S velocity:      %d kt\n", mdb->ns_vel);
	}

	if (mdb->ew_vel_valid) {
		fprintf(to, " E/W velocity:      %d kt\n", mdb->ew_vel);
	}
	...
	switch (mdb->track_type) {
		...
		case TT_MAG_HEADING:
			fprintf(to, " Heading:           %u (magnetic)\n", mdb->track);
			break;

		case TT_TRUE_HEADING:
			fprintf(to, " Heading:           %u (true)\n", mdb->track);
			break;
	...
	switch (mdb->vert_rate_source) {
		case ALT_BARO:
			fprintf(to, " Vertical rate:     %d ft/min (from barometric altitude)\n", mdb->vert_rate);
			break;

		case ALT_GEO:
			fprintf(to, " Vertical rate:     %d ft/min (from geometric altitude)\n", mdb->vert_rate);
			break;
	...
	if (mdb->dimensions_valid) {
		fprintf(to, " Dimensions:        %.1fm L x %.1fm W%s\n", mdb->length, mdb->width, mdb->position_offset ? " (position offset applied)" : "");
	}
...
/*! \brief Array of UAT emitter category names */
...
/*! \brief Array of UAT emergency statuses */
...
static void uat_display_ms(const struct uat_adsb_mdb *mdb, FILE *to) {
	...
	fprintf(to, "MS:\n"
			...
			emergency_status_names[mdb->emergency_status], mdb->uat_version, mdb->sil, mdb->transmit_mso,
			mdb->nac_p, mdb->nac_v, mdb->nic_baro, mdb->has_cdti ? "CDTI " : "", mdb->has_acas ? "ACAS " : "",
...
static void uat_display_auxsv(const struct uat_adsb_mdb *mdb, FILE *to) {
	...
	switch (mdb->sec_altitude_type) {
		case ALT_BARO:
			fprintf(to, " Sec. altitude:     %d ft (barometric)\n", mdb->sec_altitude);
			break;

		case ALT_GEO:
			fprintf(to, " Sec. altitude:     %d ft (geometric)\n", mdb->sec_altitude);
			break;

		default:
			fprintf(to, " Sec. altitude:     unavailable\n");
			break;
	}
}
