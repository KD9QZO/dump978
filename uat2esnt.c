...
// If you call this with constants for firstbit/lastbit, gcc will do a pretty good job of crunching it down to just a
// couple of operations. Even more so if value is also constant.
...
static int encode_cf(struct uat_adsb_mdb *mdb) {
	// Encode the CF field for DF 18; this says whether this is a TIS-B message, an ADS-B rebroadcast, etc
...
static int encode_imf(struct uat_adsb_mdb *mdb) {
	// Encode the IMF bit for DF 18; this is 0 if the address is a regular 24-bit ICAO address, or 1 if it uses a different format.
	switch (mdb->address_qualifier) {
		case AQ_ADSB_ICAO:
		case AQ_TISB_ICAO:
			return (0);
...
static inline void usage(int argc, char *argv[]) {
	fprintf(stderr, "usage: %s [-t]\n\n"
			"Reads UAT downlink messages from stdin and writes ADS-B ES/NT messages (1090MHz-style) to stdout.\n\n"
